...
import { getReservedBalance } from '../services/walletApi';

const mockEscrowHoldings = [
  {
    id: 1,
    itemName: 'Vintage Rolex Submariner',
    thumbnail: 'https://images.unsplash.com/photo-1523170335258-f5ed11844a49?w=200',
    amount: 15420,
...
export function EscrowHoldings() {
  ...
  const fetchReserved = useCallback(async () => {
    if (!user) return;
    try {
      const data = await getReservedBalance(user.id);
  ...
  const totalEscrow = reservedBalance !== null ? reservedBalance : 18620; // fallback to mock
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="glass-card rounded-3xl divide-y divide-white/30 overflow-hidden shadow-sm border border-white/40">
              ...
              <span
                className={`px-3 py-1 rounded-full text-sm font-medium border ${
                  holding.status === 'active' 
                    ? 'bg-slate-800/15 text-slate-800 border-slate-800/20' 

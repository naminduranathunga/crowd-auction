...
export function EditListing() {
  ...
  const { id } = useParams();
  const [title, setTitle] = useState('Limited Edition Sneakers');
  const [description, setDescription] = useState(
    'Rare collectible sneakers in mint condition'
  );
  const [category, setCategory] = useState('fashion');
  const [startingPrice, setStartingPrice] = useState('500');
  const [startDateTime, setStartDateTime] = useState('2026-05-13T10:00');
    ...
    <div className="min-h-screen">
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <button
            type="submit"
            className="w-full px-6 py-3 bg-slate-800 text-white rounded-full font-semibold hover:bg-slate-900 transition-colors">
